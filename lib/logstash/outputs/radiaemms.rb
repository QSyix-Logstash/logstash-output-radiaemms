...
# This plugin takes the event message stream, formats it suitable for
# Radia Messaging Server edmxmlobj format. And finally sends it to Radia Messaging Server
# as http post request.
# This plugin is cloned from logstash-output-http plugin. Logstash-output-http plugin
# doesn't support codecs yet, once the codec functionality is supported we can use
# the code plugin instead of this full http client.
#
...
# }
#
class LogStash::Outputs::Radiaemms < LogStash::Outputs::Base
  ...
  config :format, :validate => ["json", "json_batch", "form", "message", "xmledmobj"], :default => "json"
  ...
  # xml object rmp version
  config :xmlobj_rmpversion, :validate => :string, :default => "2.0"

  # xml object name
  config :xmlobj_name, :validate => :string, :required => :true
  ...
  def register
    ...
    if @content_type.nil?
      case @format
        ...
        when "xmledmobj" ; @content_type = "text/xml"
      end
    end
  ...
  def event_body(event)
    ...
    elsif @format == "xmledmobj"
      encode_xmledmobj(map_event(event))
  ...
  def encode_xmledmobj(hash)
      builder = Nokogiri::XML::Builder.new do |xml|
          xml.RMP('VERSION' => @xmlobj_rmpversion ) {
              xml.OBJECT('NAME' => @xmlobj_name.upcase) {
                  xml.HEAP() {
...
end # class LogStash::Outputs::Radiaemms
